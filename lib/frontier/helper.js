...
const { cmodePO, cmodePAL, cmodePSD, cmodSite, cmodePSSD } = require('./modes')
...
class FrontierHelper {
  ...
  static shouldAddToFrontier (url, curURL, tracker) {
    if (tracker.mode === cmodePSD || tracker.mode === cmodSite || tracker.mode === cmodePSSD) {
  ...
  static shouldAddToFrontierPSD (url, curURL, tracker) {
	...
	let tdTest = false;
	if(tracker.mode === cmodePSSD){
		console.log("Check subd: " + cDomain.subdomain + " (" + curURL + ") vs " + td.subdomain + " (" + url.href + ")" );
		tdTest = td && cDomain.domain === td.domain && cDomain.subdomain === td.subdomain
		console.log("td test : " + tdTest);
	} else {
		tdTest = td && cDomain.domain === td.domain
	}
